...
\documentclass{vldb}
\usepackage{etex} % Otherwise we might run into problems of the form "No room for a new \dimen"
\usepackage{times}
\usepackage{graphicx}
...
functions). Finally, since Greenplum retains most of the extension interfaces exposed by PostgreSQL, the current MADlib portability interfaces (e.g., the C++ abstraction layer for UDFs in Section~\ref{}) will likely require revisions when porting to a system without PostgreSQL roots.
...
\end{enumerate}
Clearly, a user-defined aggregate is inherently data-parallel if it has what Gray, et al. call the Distributive property~\cite{datacube}: the transition function is associative and the merge function returns the same result as if the transition function was called repeatedly for every individual element in the second state.
...
$m$-of-$n$ Bootstrap sampling in the original MAD Skills paper~\cite{MADSkills}.  It is supported in some fashion in a variety of DBMSs, sometimes by writing a simple table function consisting of a few lines of code.
...
Wang et al.\ took
this approach to implement in-database MCMC inference~\cite{Daisy11}.  Unfortunately the level of support for window aggregates varies across SQL engines.
...
In an upcoming paper we report initial experiments showing that our IGD based approach
