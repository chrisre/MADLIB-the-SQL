...
\begin{enumerate}
	...
	\item An optional \emph{merge function} that takes two transition states and transforms them into a new combined transition state. This function is only needed for parallel execution.
...
Here $D$ and $\vec z_m$ are transformations of $X$ and $\widehat{\vec \beta}_m$.
...
\end{math}
This problem is known to be NP-hard for arbitrary $d$ and $k$ \cite{ADH09a,MNV10a}. 
...
We can store points in a table called \texttt{points}, a \texttt{coords} attribute containing the points' coordinates, and the current \texttt{centroid\_id} for the point as a second attribute.  Given an array of \texttt{centroids} from the iteration state, we can define a UDF \texttt{closest\_point(a,b)} that determines the point in array \texttt{a} that is closest to \texttt{b}.  Given such a table, we can make the point-to-centroid assignments explicit using the following SQL:
\begin{scriptsize}
  \lstset{language=SQL, numbers=none, frame=none}
\begin{lstlisting}
UPDATE points
...
\subsection{Infrastructure Performance Trends}
...
1960s. SGD is a refinement of gradient methods that is useful when the
...
macro- and micro-programmin described earlier are examples of design
...
MADlib v0.3 provides in-database implementations of a large number of widely-used statistical and analytic methods.  It is still in its early stages of development, but is already in use both at research universities and at customer sites.  In future we hope to foster more partnerships with academic institutions, database vendors, and customers.  In doing so, we hope to jointly pursue additional analytic methods prioritized by both research ``push'' and customer ``pull''.  We also look forward to ports across DBMSs.  

One motivation for building MADlib was to fill a vacuum on the database side of the scalable analytics open-source ecosystem.
In our experience, a parallel DBMS makes a fine dataflow substrate for implementing statistical and analytics methods at scale.  We have not yet undertaken comparisons with comparable projects over Hadoop like Mahout or SystemML.  From a marketplace perspective, such comparisons are not terribly important right now: customers currently desire both SQL and Hadoop-based analytics libraries simply because they have data in both environments.  So the best strategy currently is to foster work like MADlib and Mahout side-by-side.  From a technical perspective, performance comparisons would likely boil down to well-known (and likely temporary) tradeoffs between the current states of DBMSs and Hadoop: C vs. Java, pipelining vs. checkpointing, etc.~\cite{pavlo2009comparison}.  These tradeoffs are not endemic, and one can expect them to level off over time as the various systems expand their feature sets.
