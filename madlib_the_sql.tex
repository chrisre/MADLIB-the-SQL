...
% BEGIN Convenience commands
	...
	\newcommand{\Florian}[1]{\textbf{\color{blue}[\textsc{Florian:} #1]}}
...
\begin{itemize}
    ...
    \item \textbf{Valuable data vs.\ valuable software}:  In many emerging business sectors, the corporate value is captured in the data itself, not in the software used to analyze that data.  Indeed, it is in the interest of these companies to have the open-source community adopt and improve their software.  Open-source efforts can also be synergistic for vendors that sell commercial software to customers who run open-source code.  Most IT shops today run a mix of open-source and proprietary software, and it is wise for software vendors to position themselves intelligently in that context.  For many database system vendors, their core competency is not in statistical methods, but rather in the engines that support those methods, and the service industry that evolves around them. \Florian{As a reader I ask ask myself here: Does Greenplum have competency in statistical methods? If so, why do they want to let competitors benefit from that? The argument would be plausible if MADlib was a joint venture, but leaves open questions as a unilateral move by Greenplum.}
    \item \textbf{Closing the research-to-adoption loop}:  Very few companies that depend on data analysis have the capacity to develop significant in-house research into computing or data science. \Florian{Again, one would assume that the Greenplum side of MADlib could not care less if other companies are lacking certain capacities.} On the other hand, it is hard for academics doing computing research to understand and influence the way that analytic processes are done in the field.  An open source project like MADlib has the potential to connect academic researchers not only to industrial software vendors, but also directly to the end-users of analytics software.  This can improve technology transfer from academia into practice without requiring database software vendors to serve as middlemen.  It can similarly enable end-users in specific application domains to influence the research agenda in academia.
    \item \textbf{Leveling the playing field, encouraging innovation}:  Over the past two decades, database software vendors have developed proprietary data mining toolkits consisting of textbook algorithms.  It is hard to assess their relative merits.  Meanwhile, other communities in machine learning and internet advertising have also been busily innovating, but their code is typically not well packaged for reuse, and the code that is available was not written to run in a database system.  Meanwhile, none of these projects has demonstrated the vibrancy and breadth we see in the open-source community surrounding R and its CRAN package.  A robust open-source project like MADlib \Florian{We should find the right mix of self-confidence and at the same time acknowledging that MADlib is still at an early stage and beta in many respects.} can bring the entire database community up to a baseline level of competence on standard statistical algorithms, remove the corporate \textit{FUD}  from proprietary toolkits that has held back innovation, and help focus a large community on innovation and technology transfer.
\end{itemize}
...
pre-existing commercial efforts. \Florian{I think Linux is too big a project to say it does not contain research. This paragraph also sounds a bit too negative to me. Like ``Today's OSS consists mainly of boring rip-offs of commercial products.'' While not completely untrue, there is more to the story.}

MADlib is making an explicit effort to back up academic research with professional software engineering.  Many academic research projects are supported by financial grants and gifts from companies.  In MADlib, the corporate donation consists of significant work hours from professional software engineers.  \Florian{This paragraph sounds as if MADlib is contrasting, say, Linux here in that (a) it contains cutting-edge research and (b) corporate donations consist mainly of labor instead of money. I do not see that MADlib is singular in that respect. Think of what, e.g., RedHat does, or in fact any company that makes money with GPL'd software.} This leverages a strength of industry that cannot be replicated on a university
...
academic open-source research, and speed technology transfer to industry. \Florian{Linux is used and advanced by lots of students and academics, and there are companies like RedHat, IBM, etc.}
...
has room to mature. \Florian{I agree.} There is room for enhancements in its core
...
for QA support on additional database engines). \Florian{True for pure SQL modules. For pgSQL/C modules, porting at this point is essentially rewriting. And for C++ AL modules, porting is a non-trivial rewrite of the AL.}
...
It has been observed before that computing $\widehat{\vec b}$ lends itself well to data-parallel implementations \cite{DBLP:conf/nips/ChuKLYBNO06}. In the following, we take OLS linear-regression as an example for discussing MADlib's internal implementation in detail. The principal observation is this: Since $X^TX = \sum_{i=1}^n \vec x_i \vec x_i^T$ and $X^T \vec y = \sum_{i=1}^n \vec x_i y_i$ are just sums of transformations of each data point, we can compute these sums in a single pass over the data. In fact, since summation is associative, data parallelism virtually comes for free: We can compute the per-segment subsums of the previous expressions locally on each segment, and then sum up all subsums during a second-phase aggregation. As a final non-parallelized step, we compute the (pseudo\nobreakdash-)inverse of $X^TX$ and then multiply with $X^T \vec y$. These final operations are comparatively cheap, since the number of independent variables (and thus the dimensions of $X^T X$ and $X^T \vec y$) is typically ``small''.

\subsubsection{MADlib Implementation}

The above description directly translates into an implementation through a user-defined aggregate, which (in PostgreSQL/Greenplum terminology) consists of:
\begin{enumerate}
	\item A \emph{transition function} that takes the current transition state and a new data point. Its output is the combined transition state. The transition function is equivalent to the ``combining'' function passed to \emph{left-fold} functions in functional-programming languages.
...
\end{enumerate}
While the fold-paradigm is natural, and arguably ubiquitous for implementing aggregates (MySQL, PostgreSQL, Map-Reduce, etc.), the SQL standard does not prescribe an extension interface, and user-defined aggregates are entirely vendor-specific. For MADlib, we therefore implemented an abstraction layer that facilitates encapsulating user-defined-function logic from DBMS logic or its APIs. The design was guided by the four requirements correctness, robustness, performance, and maintainability/portability. These requirements manifest in the code as follows---with the following not meant to be exhaustive. See Listings~\ref{fn:lin-reg-trans} and \ref{fn:lin-reg-final} for reduced, but fully functional, excerpts from the MADlib source code.
\begin{itemize}
	\item MADlib does not reinvent the wheel but uses proven third-party party libraries for linear-algebra code and other mathematical code. (All requirements)
	\item MADlib aggregates are written in C or (preferably) C++. Arguments are passed by reference to immutable data structures. (Performance)
	\item Mutable data structures are provided for cases where data can be safely modified in-place. Should a DBMS not provide mutable values, the abstraction layer creates mutable copies transparently. (Performance)
	\item By virtue of being a template library, the runtime and abstraction overhead is reduced to a minimum. (Performance)
	\item The C++ abstraction layer provides a complete encapsulation, and no DBMS-specific API is used. (Maintainability)
	\item To the greatest extend supported by the DBMS, the abstraction layer catches all type errors and facilitates writing strictly type-safe code. (Robustness)
\end{itemize}
...
Given the user-defined aggregate, running MADlib's linear regression takes a mere SQL statement of form ``\texttt{SELECT madlib.linregr(y, x) FROM data}''.
...
Obviously, each individual iteration can be implemented via a user-defined aggregate using linear regression as a blueprint. However, the handling of iterations requires a further outer loop, which we call driver. Unfortunately, the most obvious implementations, via recursive queries or multi-pass user-defined aggregates lack wide-spread support in today's DBMSs.

The only portable option left is thus to drive iterations from within a user-defined function. For portability we chose to implement such driver function in Python. See Listing~\ref{fn:log-reg-driver}. \todo{To be completed.}
		...
		return __runIterativeAlg(
		    ...
		    updateExpr = """
		        ...
		        """.format(
		            ...
		            optimizer = optimizer),
		    terminateExpr = """
		        {MADlibSchema}.internal_logregr_{optimizer}_step_distance(
		            {{newState}}, {{oldState}}
		        ) < {precision}
		        """.format(
		            MADlibSchema = MADlibSchema,
...
Example k-means:
\begin{itemize}
	\item even though the current state is completely captured by the set of centroids, we want to cache distances
	\item Won't fit into memory
	\item Need to resort to temporary tables.
\end{itemize}

\subsection{MADlib Performance}

\todo{To be completed. Might remove comments about the reference BLAS because anybody with HPC background is well aware that this is not a tuned library.}
Our runtime measurements fit will to the theoretical asymptotical runtime behavior. Roughly, the relevant terms are (as expected)
\begin{gather*}
	a \cdot k^3 + \frac 1p \cdot (b \cdot n \cdot k^2 + c)
\end{gather*}
where $k$ is the number of independent variables, $n$ is the number of rows, $p$ is the number of segments, and $a$, $b$, and $c$ are constants. \todo{Substantiate. Determine values for $a,b,c$.} In particular: If the quadratic term dominates (the number of independent variables is less than a couple of hundred), Greenplum has a linear speed-up in the number of segments.

On current hardware, computing the pseudo-inverse with libraries such as Eigen, or LAPACK/BLAS is in the order of more than a minute if the number of independent variables reaches one thousand. In that case, the cubic term often dominates.
