...
\author{Joseph M. Hellerstein\\{\small  hellerstein@berkeley.edu}\\{\small U.C. Berkeley} \and
 Christoper R\'{e}\\{\small chrisre@cs.wisc.edu}\\{\small U. Wisconsin} \and
 Florian Schoppmann\\{\small Florian.Schoppmann@emc.com}\\{\small Greenplum} \and
 Daisy Zhe Wang\\{\small daisyw@cise.ufl.edu}\\{\small U. Florida} \and
 % \and
 % Gavin Sherry\\{\small Greenplum} \and
 Eugene Fratkin\\{\small Eugene.Fratkin@emc.com}\\{\small Greenplum} \and
 Aleksander Gorajek\\{\small Aleksander.Gorajek@emc.com}\\{\small Greenplum} \and
 Kee Siong Ng\\{\small KeeSiong.Ng@emc.com}\\{\small Greenplum} \and
 Caleb Welton\\{\small Caleb.Welton@emc.com}\\{\small Greenplum} \and
 Xixuan Feng\\{\small xfeng@cs.wisc.edu} \\ {\small U. Wisconsin} \and
 Kun Li\\{\small kli@cise.ufl.edu} \\{\small U. Florida} \and
 Arun Kumar\\{\small arun@cs.wisc.edu} \\{\small U. Wisconsin} \and
...
%  \jmh{Care to speculate why?} \fs{Pure speculation at this stage: Memory speed does not scale similarly, non-linear parts of the code/branches, different pipeline lengths, different number of clock cycles per FP operation, ...?}
...
\begin{figure}
\centering
\includegraphics{linregr-diagram}
\vspace{-2em}
...
techniques to reduce this burden.  Our approach is to expose a mathematical abstraction on top of MADlib
that allows a developer to specify a smaller amount of code, which we
hope will lower the development time to add new techniques in many cases. We discuss
the challenge that we faced implementing this abstraction. 
% \fs{Maybe make clear already here that this is a mathematical generalization, not a software-engineering abstraction (like the C++ AL)?} 
To
demonstrate our ideas, we have implemented all of the models shown in
Table~\ref{fig:objs:grads} within the single abstraction (built within
...
abstraction called convex optimization 
% \fs{The previous section mentions `convex program' without any explanation.} 
...
% \fs{I would like a comment on the overlap of linear and logistic regression from the previous section.}
...
solution. 
% \fs{Minor detail: Can we phrase the previous sentence in a mathematically exact way? Of course, just because a monotonically decreasing sequence is bounded from below by some number does not yet imply convergence against that number.} 
One such popular greedy method is called a gradient
...
provable rate of convergence to the minimum value, which is in
sharp contrast to a typical greedy search.  In our prototype
implementation in MADlib, we picked up one such simple greedy
algorithm, called stochastic (or sometimes, ``incremental'') gradient descent
(SGD)~\cite{RobbinsMonro51,BertsekasNLP}, that goes back to the
1960s. SGD is an approximation of gradient methods that is useful when the
convex function we are considering, $f(x)$, has the form:
 
\[ f(x)= \sum_{i=1}^N f_i (x) \]

If each of the $f_i$ is convex, then so is
$f$~\cite[pg.~38]{boyd:cvx}. Notice that all problems in Table~\ref{fig:objs:grads} are
...
examples. SGD leverages the above form to construct a rough estimate
...
\begin{equation}
x \leftarrow x - \alpha N \cdot G_i(x)   
\label{eq:incremental}\end{equation}
This approximation is guaranteed to converge to an optimal solution~\cite{Nedic00}. 
% \fs{We should have the reference here.}

 
\paragraph*{Using the MADlib framework}
In our setting, each tuple in the input table for an analysis task
encodes a single $f_i$. We use the {\em micro-programming} interfaces of
Sections~\ref{sec:micro} and~\ref{sec:cplusplus} to perform the mapping from the tuples to the
...
Table~\ref{fig:objs:grads} in a matter of days.
...
%% SGD algorithm as a standard user-defined aggregation function: the
...
%% SGD is one of a spectrum of gradient methods that differ in how they
%% approximate they gradient term above: at one extreme is SGD where we
...
%% That said, our initial experiments show that our SGD based approach
...
\hline
\end{tabular}}
\caption{Models currently Implemented in MADlib using the SGD-based
  approach. 
...
%% \begin{enumerate}
%% \item \textbf{Improving Performance}. SGDs are not a panacea. They
...
\subsection{Florida/Berkeley Contributions: \\Statistical Text Analytics}
\label{sec:viterbi}
\label{sec:mcmc}
%Importance of statistical text analysis 
The focus of the MADlib work at Florida and Berkeley has been to integrate statistical text analytics into a DBMS.
In many domains, structured
data and unstructured text are both important assets for data
analysis. For example, electronic medical record systems use transactional databases to store 
both structured data such as lab results and monitor readings, and unstructured data such as notes from physicians. 
With our goal to push analysis close to the data, it is important for MADlib to support basic statistical 
methods for data scientists to implement text analysis tasks.

% different text analysis tasks
Basic text analysis tasks include part-of-speech (POS) tagging,
named entity extraction (NER), and entity resolution (ER)~\cite{feldman2007text,JurafskyMartin}.
...
(CRFs)~\cite{lafferty2001conditional}.
...
features depends on the application, and so leveraging the micro-programming
 interface, we implemented a common set of text feature extractors.
...
text string. Using the 3-gram index, we created an approximate matching UDF 
that takes in a query string and returns all documents in the corpus that contain at 
...
programming algorithm is a popular algorithm to find
the top-k most likely labelings of a document for (linear chain) CRF
models~\cite{viterbi}. 
% The following dynamic programming computes the top-1 label
...
\paragraph*{Using the MADlib Framework} 
Because this work predated the release of MADlib, it diverged from the macro-coordination patterns of Section~\ref{sec:macro}, and took advantage of  PostgreSQL features that are less portable.  These details require refactoring to fit into the current MADlib design style, which we are currently working on.
...
implementations, but runs natively in a DBMS close to the data~\cite{DBLP:journals/pvldb/WangFGH10,wang2011hybrid}. 
...
data processing substrate. Specifically, we split the approaches to
...
Framework-based ap\-proach\-es attempt to provide the analyst with
...
Mahout provides a library of machine learning algorithms and a
...
techniques. 
SciDB advocates a completely rewritten DBMS engine for numerical computation, arguing that RDBMSs have ``the wrong data model, the wrong operators, and are missing required capabilities''~\cite{scidb}.  MADlib may be seen as a partial refutation of this claim. Indeed, the assertion in~\cite{scidb} that ``RDBMSs, such as GreenPlum~[sic]...must convert a table to an array inside user-defined functions'' is incorrect: the MADlib C++ library hides representation from the UDF developer and only pays for copies when modifying immutable structures. 
GraphLab is a framework to simplify the design of
...
cluster setting. 
Another popular toolkit in this space is
Vowpal Wabbit~\cite{vw} that is extensively used in the academic
machine learning community and provides
high performance.

Two recent efforts to provide Scala-based domain-specific languages (DSLs) and parallel execution frameworks blur distinctions between frameworks and language-based approaches. Spark~\cite{Zaharia:EECS-2011-82} is a Scala DSL targeted at Machine Learning, providing access to the
fault-tolerant, main-memory {\em resilient distributed datasets}:
which are read-only collections of data that can be partitioned across
a cluster. ScalOps~\cite{scalops} provides a Scala DSL for Machine Learning that is translated to Datalog, which is then optimized to run in parallel on the Hyracks infrastructure~\cite{hyracks}.  Given its roots in Datalog and parallel relational algebra, ScalOps bears more similarity to MADlib than any of the other toolkits mentioned here.  It would be interesting to try and compile its DSL to the MADlib runtime.
...
% More interesting questions arise by focusing a level up, at the language layer.  MADlib's use of Python, SQL, and C++ abstractions for algebra reflect the realities of the SQL development ecosystem, rather than an idealized language design.  Perhaps a better approach would be to design a clean high-level language for machine learning, and compile down to the pragmatic mix that works on a contemporary DBMS.  systemml and ScalOps take this approach for MapReduce.  More interestingly, one could try to {\em cross-compile} such a language to multiple platforms---including not only the well-known parallel dataflow platforms (DBMSs, MapReduce) but also specialized machine learning engines like GraphLab that are architected quite differently.  Doing so would not only unify the open source ecosystem for analytics around a single language, but could also highlight important tradeoffs between these very different architectures for parallelism.
