...
Data scientists make use of database engines in a very different way than traditional practice in data warehousing.  Rather than carefully
...
ports are provided in Table~\ref{tab:methods} and Table~\ref{tab:ports}.
...
Regardless of the adoption curves of these new systems, large volumes of valuable data are likely to reside in
relational databases for some time to come. There is a rich
ecosystem of tools, know-how, and organizational requirements that
encourage this.  For these cases, it would be helpful to push statistical methods into the DBMS. As we will see, massively parallel databases form a surprisingly useful platform for sophisticated analytics. 
...
\label{sec:gen}
The core of traditional SQL---\texttt{SELECT... FROM... WHERE... GROUP BY}---is
...
computing on these issues that can inform in-database implementations~\cite{demmel}.  Once data is properly partitioned, database engines shine at orchestrating the
...
limitations in standard SQL.  In both cases we solved our problems using driver code written in simple
...
\textbf{Example: Linear Regression Background}
